...
FROM --platform=${BUILDPLATFORM} rust:1.94.1-slim-trixie@sha256:1d0000a49fb62f4fde24455f49d59c6c088af46202d65d8f455b722f7263e8f8 AS rust-base
...
# front-end (NPM) build
FROM --platform=${BUILDPLATFORM} node:24.14.1-alpine3.22@sha256:4f33c7804fa7775b87875f512e25e515692210bfdeafd715eb8bc441c364e2f6 AS typescript-build
...
# That means that if our dependencies don't change rebuilding is much faster
WORKDIR /build
COPY package.json pnpm-lock.yaml pnpm-workspace.yaml vite.config.ts tailwind.config.mjs tsconfig.json ./
